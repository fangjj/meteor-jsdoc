...
Actions.prototype.build = function() {
  ...
  if (self.config.preamble) {
    ...
    if (fse.existsSync(destPreamble)) {
      var log = `Skipped ${fileName} copy to avoid overwriting.`;
...
Actions.prototype.start = function() {
  var self    = this;
  var cmdnpm  = `meteor npm install`;
  var cmd     = `meteor --port ${self.config.meteorPort} > app.log 2>&1 &`;

  exec(cmdnpm, { cwd: self.config.docsPath }, (error, stdout, stderr) => {
    if (error !== null) {
      helpers.error(error);
    } else {
      helpers.success(`meteor npm install -> success`);
    }
  });
